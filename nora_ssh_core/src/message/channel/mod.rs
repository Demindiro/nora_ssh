...
impl<'a> Open<'a> {
	fn parse(data: &'a [u8]) -> Result<Self, OpenParseError> {
		...
		if data.len() < 12 {
			...
			Ok(Self { ty, sender_channel, window_size, max_packet_size })
		}
	}
...
impl WindowAdjust {
	fn parse(data: &[u8]) -> Result<Self, WindowAdjustParseError> {
		...
		data.is_empty()
			.then(|| Self { recipient_channel, add_bytes })
...
impl<'a> Data<'a> {
	fn parse(data: &'a [u8]) -> Result<Self, DataParseError> {
		...
		d.is_empty()
			.then(|| Self { recipient_channel, data })
...
impl<'a> ExtendedData<'a> {
	fn parse(data: &'a [u8]) -> Result<Self, ExtendedDataParseError> {
		...
		d.is_empty()
			.then(|| Self { recipient_channel, ty, data })
...
impl<'a> Request<'a> {
	fn parse(data: &'a [u8]) -> Result<Self, RequestParseError> {
		...
		Ok(Self { recipient_channel, ty, want_reply, data: &data[1..] })
	}
