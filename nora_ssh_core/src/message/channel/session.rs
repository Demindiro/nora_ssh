...
impl<'a> SessionRequest<'a> {
	pub fn parse(ty: &'a [u8], data: &'a [u8]) -> Result<Self, ParseError> {
		...
		match ty {
			...
			b"signal" => {
				...
				empty(data).map(|()| Self::Signal { signal: signal.into() })
			}
			b"exit-status" => {
	...
	pub fn serialize<'b>(&self, buf: &'b mut [u8]) -> Option<(&'static [u8], usize)> {
		match self {
			Self::Pty { terminal, char_width, char_height, pixel_width, pixel_height, modes } => {
			...
			Self::X11 { single_connection, auth_protocol, auth_cookie, screen } => {
			...
			Self::WindowChange { char_width, char_height, pixel_width, pixel_height } => {
			...
			Self::ExitSignal { signal, core_dumped, message, language } => {
